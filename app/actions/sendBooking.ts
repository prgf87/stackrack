...
export async function sendBookingEnquiry(data: BookingFormData) {
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a0a0f;color:#ccc;padding:32px;border:1px solid #222">
          <h2 style="color:#a855f7;font-size:20px;margin:0 0 24px">New Booking Enquiry</h2>
          <table style="width:100%;border-collapse:collapse;font-size:14px">
            <tr><td style="padding:8px 0;color:#888;width:140px">Name</td><td style="padding:8px 0;color:#fff">${data.name}</td></tr>
            <tr><td style="padding:8px 0;color:#888">Email</td><td style="padding:8px 0;color:#fff"><a href="mailto:${data.email}" style="color:#a855f7">${data.email}</a></td></tr>
          ...
          ${data.message ? `<div style="margin-top:20px;padding:16px;background:#111;border-left:2px solid #a855f7;font-size:14px;color:#ccc">${data.message}</div>` : ''}
        </div>
      `,
    });
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a0a0f;color:#ccc;padding:32px;border:1px solid #222">
          <h2 style="color:#a855f7;font-size:20px;margin:0 0 16px">Thanks, ${data.name}.</h2>
          ...
          <p style="font-size:13px;color:#666;margin:24px 0 0;border-top:1px solid #1a1a1a;padding-top:16px">
            Stack Rack · <a href="mailto:stackrack@live.com" style="color:#a855f7">stackrack@live.com</a>
          </p>
        </div>
      `,
    });
